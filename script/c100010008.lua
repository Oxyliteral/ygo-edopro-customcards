...
function s.initial_effect(c)
	...
	e2:SetCountLimit(1)
	e2:SetTarget(s.sphdtg)
	e2:SetOperation(s.sphdop)
	c:RegisterEffect(e2)
...
function s.sphdfilter(c,e,tp)
	return c:IsSetCard(SET_HERO) and c:IsLevelBelow(7) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.sphdtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.sphdfilter,tp,LOCATION_HAND,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_HAND)
end
function s.sphdop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,s.sphdfilter,tp,LOCATION_HAND,0,1,1,nil,e,tp)
	if #g>0 then
